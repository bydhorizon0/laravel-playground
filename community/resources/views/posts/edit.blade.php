...
<div class="max-w-4xl mx-auto space-y-4">
    <!-- 뒤로 가기 버튼 -->
    <div>
        <a href="{{ route('posts.show', $post) }}" class="text-sm text-gray-600 hover:text-gray-900 flex items-center gap-1">
            &larr; 게시글로 돌아가기
        </a>
    </div>

    <div class="bg-white p-6 sm:p-8 rounded-lg shadow-sm border border-gray-200">
        <h1 class="text-2xl font-bold text-gray-900 mb-6 pb-3 border-b border-gray-100">게시글 수정</h1>
        ...
        <form action="{{ route('posts.update', $post) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                >
                    @foreach($categories as $category)
                        <option value="{{ data_get($category, 'id') }}" {{ old('category_id', $post->category_id) == data_get($category, 'id') ? 'selected' : '' }}>
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
                <a href="{{ route('posts.show', $post) }}" class="bg-gray-100 text-gray-700 px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-200 transition">
                    취소
                </a>
                <button
                        type="submit"
